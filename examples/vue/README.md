...
Minimal Vite + Vue 3 + TypeScript app demonstrating all three features of `@smoker_winston/egov-helper`.
...
The library is wired in with a local file dependency (`"@smoker_winston/egov-helper": "file:../.."`) so any change you make to the library and `npm run build` is picked up automatically by Vite.
